...
# Available chat models. Extend this list as needed.
AVAILABLE_MODELS = [
    "gpt-3.5-turbo",
    "gpt-4-turbo",
]
...
def chat_post():
    ...
    # Model selected by the user; fallback to default if missing or invalid.
    model = (data.get("model") or MODEL_NAME).strip()
    if model not in AVAILABLE_MODELS:
        logger.debug(f"Invalid model '{model}' requested; using default {MODEL_NAME}.")
        model = MODEL_NAME
